...
public class ListEx1 {
	...
	public static void main(String[] args) {
		...
		// > 타입의 최상위 클래스인 Object가 자동으로 지정됨.
		// > get()을 이용할 때 클래스 형변환을 해줘야함.
		
		// - get(번) : 리스트에서만 제공하는 메서드.
		 ...
		 * - 이 때, Objects.equals() 를 이용하여 동작함.
		 * - 컬렉션에 있는 타입과 객체의 타입이 다르면 false를 반환
		 * - 컬렉션에 있는 타입과 객체의 타입이 같으면 객체.equals()를 호출해서 결과를 리턴
		 * - 해당 클래스의 equals가 어떻게 오버라이딩 되어 있느냐에 따라 결과가 달라짐.
		 * */
		System.out.println(list5.contains("abc"));
		
		/* indexOf(객체): List에서 제공해주는 메서드(Set에서는 사용할 수 없음.)
		 ...
		 * - 객체와 일치하는 요소를 제거한 후, 제거 여부를 반환함(boolean)
		 * */
		list.remove(0);
		list.remove("def");
...
//==============================================================
		...
		/* forEach(): Collection에서 제공해주는 메서드
		 * - 람다식을 이용하여 컬렉션에서 하나 꺼낸 값을 활용할 때 사용함.
		 * */
		list5.forEach(a->System.out.println(a));
		
		/* sort(): List에서 제공하는 메서드. (Set에서는 사용할 수 없음)
		 * - 람다식을 이용해 정렬 기준을 정해주면, 해당 기준대로 리스트를 정렬함.
		 * - 정렬 기준은 정수로 알려주고, 음수면 앞, 0이면 같은 위치, 양수면 뒤.
		 ...
		 * - 리스트의 타입이 comparable 인터페이스를 구현한 구현 클래스인 경우, 해당 기준을 이용해 정렬
		 * 
		 * */
