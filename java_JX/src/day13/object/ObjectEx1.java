package day13.object;

import java.util.Objects;

import lombok.AllArgsConstructor;
import lombok.EqualsAndHashCode;

public class ObjectEx1 {

	public static void main(String[] args) throws CloneNotSupportedException {
		A a = new A();
		System.out.println(a.toString());

		Student std1 = new Student(1, 1, 1, "홍길동");
		Student std2 = new Student(1, 1, 1, "임꺽정");
		
		if(std1.equals(std2)) {
			System.out.println("같ㅌ습니다.");
		}else {
			System.out.println("다릅니다");
		}
		System.out.println();
		
		A a1 = new A();
		a1.num1 = 20;
		// a1 출력
		System.out.println(a1);
		
		A a2 = a1;
		a2.num1 = 30;
		// a2의 num1을 수정한 후 a1 출력 (공유하기 때문에 값이 바뀜
		System.out.println(a1);
		
		A a3 = a1.clone();
		a3.num2 = 40;
		// a3의 num2를 수정한 후 a1을 출력함.
		System.out.println(a1);
	}

}

@EqualsAndHashCode // 전체를 기준으로 equals를 제작.
class A implements Cloneable{
	int num1 = 10;
	int num2 = 3;
	String name = "테스트";
	@Override
	public String toString() {
		return "A [num1=" + num1 + ", num2=" + num2 + ", name=" + name + "]";
	}
	@Override
	public A clone() throws CloneNotSupportedException{
		return (A)super.clone();
	}
}

@AllArgsConstructor
class Student{
	int grade, classNum, num;
	String name;
	@Override
	public int hashCode() {
		return Objects.hash(classNum, grade, num);
	}
	@Override
	public boolean equals(Object obj) {
		if (this == obj) // 같은 인스턴스를 공유하는 지 주소를 확인함. (기본 equals 기능)
			return true;
		if (obj == null) // 비교 대상이 null인지 확인함.
			return false;
		if (getClass() != obj.getClass()) // 다른 클래스의 인스턴스인 경우.
			return false;
		Student other = (Student) obj; // 다운캐스팅 (클래스 형변환)
		
		// 학년, 반, 번호가 모두 같은지 확인함.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
		return classNum == other.classNum && grade == other.grade && num == other.num;
	}
	
}