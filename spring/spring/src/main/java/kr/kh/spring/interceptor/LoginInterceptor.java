	...
	public void postHandle(
		...
		if(user != null) {
			...
			if(user.isAutoLogin()) {
				...
				// 쿠키를 생성해서
				Cookie cookie = new Cookie("loginCookie", value);	// 쿠키 이름을 loginCookie로 설정.
				cookie.setPath("/");			// 모든 경로에서 유효한 쿠키
				int time = 7 * 24 * 60 * 60;	// 일주일
				cookie.setMaxAge(time);			// 유효시간 (초단위)
				
				// 화면에 전송
				response.addCookie(cookie);
				
				// DB에 관련 정보를 추가
				user.setMe_cookie(value);	// 세션 아이디
